...
```{r plot horses, fig.width=10, fig.height=3, fig.cap = "Figure 1", fig.align = 'center'}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
knitr::kable( max_horses, caption = 'Table 1: Range of number of horses for each provinces')
```

Below we zoom in and look at the province of `r params$province`:

```{r plot province, fig.width=5, fig.height=4, fig.cap = "Figure 2", fig.align = 'center'}
